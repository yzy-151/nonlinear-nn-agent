...
REAL_QUERIES = [
    ("实验中断后如何从上次事件继续，而不是重头再来？", ["v2.0 Runtime 可靠性", "Resume Failed Run", "SSE 是最小可用流式接口"], "sse replay last event id resume runtime reliability"),
    ("系统怎么防止同一个请求被并发执行两次？", ["v2.0 Runtime 可靠性", "Task 4：补齐本项目必要的 Runtime 可靠性"], "runtime control plane sqlite dedup atomic claim lease"),
    ...
    ("怎么把项目的教训沉淀下来，让下一个模型接手时不重新踩坑？", ["15.4 共享状态与 Memory Schema", "如何避免模型反复犯同样错误", "本次改动：Reflection 读取历史先验"], "memory knowledge base structured experience supersedes"),
    ...
    ("写报告的 Agent 有没有权利修改实验数据？", ["D. Writing Agent"], "writing agent report spec fidelity no data mutation"),
    ...
    ("改代码的 Agent 在哪个隔离环境里工作？", ["B. Coding Agent"], "coding agent isolated worktree patch"),
    ("先想清楚做什么实验再动手的 Agent 输出什么格式的计划？", ["A. Idea & Plan Agent", "仿真实验如何由 LLM 设计"], "idea plan agent hypotheses experiment dag"),
    ...
    ("接手这个项目第一步应该跑什么命令？", ["3. 接手前必须运行"], "onboarding first commands unittest benchmark dashboard"),
    ("提交代码时哪些文件绝对不能进仓库？", ["4. Git 操作规则", "v0.2 的真实链路"], "git ignore env local api key secrets"),
    ...
    ("搜索策略对比实验的结论是什么？", ["v1.9 搜索对照实验"], "search comparison optuna reflection conclusion"),
    ...
    ("v3.6.0 的记忆和知识库验收指标是什么？", ["v3.6.0：Knowledge/Memory Foundation"], "v3.6.0 memory knowledge recall citation leakage acceptance"),
]
