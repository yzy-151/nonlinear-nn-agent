...
实现说明：本实验的 LLM 策略为离线可复现的邻域采样模拟（读历史最优 + 30% 随机探索），`llm_with_reflection` 额外消费 ReflectionPolicy 事实、避免被拒/失败的 model_type。因此 `prompt_tokens / completion_tokens / estimated_cost_usd` 均为 0——这些数字不代表真实 LLM 成本，只代表该模拟器未调用 LLM API。真实 DeepSeek 调用的证据见 `docs/handoff/deepseek-continuation-plan.md`（exp016 / exp_019 案例）。
